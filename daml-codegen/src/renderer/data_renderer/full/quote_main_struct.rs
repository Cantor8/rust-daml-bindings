...
pub fn quote_daml_record_and_impl(
    ...
    let struct_tokens = quote_struct(ctx, name, &supported_fields, params);
    let new_method_tokens = quote_new_method(ctx, name, &supported_fields, params);
    let serialize_trait_impl_tokens = quote_serialize_trait_impl(ctx, name, &supported_fields, params);
    let deserialize_trait_impl_tokens = quote_deserialize_trait_impl(ctx, name, &supported_fields, params);
    quote!(
        #struct_tokens
        #new_method_tokens
...
fn quote_struct(
    ...
    let body_tokens = quote_struct_body(ctx, struct_fields);
...
fn quote_new_method(
    ...
    let struct_name_tokens = quote_escaped_ident(struct_name);
    let method_arguments_tokens = quote_method_arguments(ctx, struct_fields);
...
fn quote_serialize_trait_impl(
    ...
    let body_tokens = quote_serialize_impl_body(ctx, struct_fields);
    quote!(
...
fn quote_deserialize_trait_impl(
    ...
    let body_tokens = quote_deserialize_trait_impl_body(ctx, struct_fields);
    quote!(
...
fn quote_struct_body(ctx: &RenderContext<'_>, struct_fields: &[&DamlField<'_>]) -> TokenStream {
    let all: Vec<_> = struct_fields
        .iter()
        .map(|&field| {
            let field_label = quote_escaped_ident(field.name());
            let field_type_rendered = quote_type(ctx, field.ty());
...
fn quote_serialize_impl_body(ctx: &RenderContext<'_>, struct_fields: &[&DamlField<'_>]) -> TokenStream {
    if struct_fields.is_empty() {
        quote!(DamlValue::Record(DamlRecord::new(vec![], None::<DamlIdentifier>)))
    } else {
        let all_fields: Vec<_> = struct_fields
            .iter()
            .map(|&field| quote_declare_field_serialize_trait_impl(ctx, field.name(), field.ty()))
            .collect();
        quote!(DamlValue::Record(DamlRecord::new(vec![#( #all_fields ),*], None::<DamlIdentifier>)))
    }
}

fn quote_deserialize_trait_impl_body(ctx: &RenderContext<'_>, struct_fields: &[&DamlField<'_>]) -> TokenStream {
    if struct_fields.is_empty() {
        quote!(Ok(Self::new()))
    } else {
...
fn quote_deserialize_trait_field(ctx: &RenderContext<'_>, field: &DamlField<'_>) -> TokenStream {
...
fn quote_decl_unused_phantom_params(
    params: &[DamlTypeVarWithKind<'_>],
    struct_fields: &[&DamlField<'_>],
) -> TokenStream {
...
fn quote_init_unused_phantom_params(
    params: &[DamlTypeVarWithKind<'_>],
    struct_fields: &[&DamlField<'_>],
) -> TokenStream {
    quote_unused_phantom_params(params, struct_fields, |param| {
...
fn quote_unused_phantom_params(
    params: &[DamlTypeVarWithKind<'_>],
    struct_fields: &[&DamlField<'_>],
    type_var_quoter: impl Fn(&DamlTypeVarWithKind<'_>) -> TokenStream,
) -> TokenStream {
    let all_params: Vec<_> = params
        .iter()
        .filter(|&p| struct_fields.iter().any(|&f| f.ty().contains_type_var(p.var())).not())
        .map(type_var_quoter)
        .collect();
    quote!( #( #all_params ),* )
}
